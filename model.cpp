...
Vec3f Model::normal(Vec2f uvf) {
    if (!normalmap_.get_width()) return Vec3f(0, 0, 1);
...
float Model::specular(Vec2f uvf) {
    if (!specularmap_.get_width()) return 0.f;
    Vec2i uv(uvf[0] * specularmap_.get_width(), uvf[1] * specularmap_.get_height());
    return specularmap_.get(uv[0], uv[1])[0] / 1.f; // степень блеска
}
...
void Model::load_texture(std::string filename, const char* suffix, TGAImage& img) {
    ...
    if (dot != std::string::npos) {
        ...
        std::cerr << "texture " << texfile
            << " loading " << (img.read_tga_file(texfile.c_str()) ? "ok" : "failed")
            << std::endl;
        img.flip_vertically();
    }
}
